...
export function EditorSkeleton() {
  return (
    ...
    >
      ...
      <div className="hidden md:flex flex-col w-[72px] border-r border-gray-200 bg-gray-50 p-2 gap-2">
        {Array.from({ length: 4 }, (_, i) => (
          <div
            key={i}
            className="w-full aspect-[3/4] rounded-md bg-gray-200 animate-pulse motion-reduce:animate-none"
          />
        ))}
      </div>
      ...
      <div className="flex-1 flex flex-col relative bg-[#1a1a2e]">
        ...
        <div className="absolute top-4 left-1/2 -translate-x-1/2 z-10 flex items-center gap-1.5 px-3 py-2 bg-white/90 backdrop-blur-sm rounded-xl shadow-md">
          {Array.from({ length: 8 }, (_, i) => (
            <div
              key={i}
              className="w-8 h-8 rounded-lg bg-gray-200 animate-pulse motion-reduce:animate-none"
      ...
      <div className="hidden lg:flex flex-col w-[320px] border-l border-gray-200 bg-white p-4 gap-4">
        {/* Panel title */}
        ...
        {Array.from({ length: 3 }, (_, groupIdx) => (
          <div key={groupIdx} className="flex flex-col gap-2.5">
            <div
              className="h-3 w-16 rounded bg-gray-200 animate-pulse motion-reduce:animate-none"
              style={{ animationDelay: `${groupIdx * 100}ms` }}
            />
            {Array.from({ length: 3 }, (_, lineIdx) => (
              <div
                key={lineIdx}
                className="h-9 w-full rounded-md bg-gray-100 animate-pulse motion-reduce:animate-none"
